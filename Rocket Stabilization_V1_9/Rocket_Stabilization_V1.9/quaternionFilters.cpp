...
static float ypr[3] = {0.0f, 0.0f, 0.0f};//Yaw, Pitch Roll
...
void MadgwickQuaternionUpdate(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz, float deltat)
{
  ...
  q[2] = q3 * norm;
  q[3] = q4 * norm;
  ypr[0]   = atan2(2.0f * (q[1] * q[2] + q[0] * q[3]), q[0] * q[0] + q[1] * q[1] - q[2] * q[2] - q[3] * q[3]);   //convert to yaw, pitch and roll
